...
describe('authTokenInterceptor', () => {
  ...
  it('adds the bearer token to cart API requests when a session exists', () => {
  ...
  it('skips API requests when the persisted session is malformed or unauthenticated', () => {
    localStorage.setItem(SESSION_STORAGE_KEYS.AUTH_SESSION, '{bad-json');
    http.get(`${environment.ecommerceApiBaseUrl}/cart`).subscribe();

    const malformedRequest = httpTesting.expectOne(`${environment.ecommerceApiBaseUrl}/cart`);
    expect(malformedRequest.request.headers.has('Authorization')).toBe(false);
    malformedRequest.flush([]);

    localStorage.setItem(
      SESSION_STORAGE_KEYS.AUTH_SESSION,
      JSON.stringify({ isAuthenticated: false, token: null, user: null }),
    );
    http.get(`${environment.ecommerceApiBaseUrl}/cart`).subscribe();

    const unauthenticatedRequest = httpTesting.expectOne(`${environment.ecommerceApiBaseUrl}/cart`);
    expect(unauthenticatedRequest.request.headers.has('Authorization')).toBe(false);
    unauthenticatedRequest.flush([]);
  });
});